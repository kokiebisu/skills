...
- If this file doesn't exist when `/onboard` needs it, create it with defaults (`docLanguages` = the language of the current conversation, `docsDirectory` = `docs/onboarding`, `commitByDefault` = `true`) and mention once, briefly, that `/setup-onboard` exists if the team wants to set these explicitly instead — but don't block on it, just create the defaults and continue.
